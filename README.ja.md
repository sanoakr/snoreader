...
- キーワードによる自動タグ付けは明示操作でのみ行い、保管の副作用としては付けない。スターを付けてもタグは増えず、同じマッチ結果はリーダーの青い「Suggested:」チップに出るのでタップ 1 回で付けられる。「タグ」⚙ のモーダルの `キーワードで付与` は Saved 記事に対して一括でマッチを走らせる（未タグは最大 3 件付与、4 件以上付いている記事は既存タグを剥がして再付与）。隣の `AI でタグを生成` は LLM の経路（タグの無い Saved 記事を 1 回 10 件、新しいタグ名も作る）、`日本語名を補完` は日本語名の穴埋め。各ボタンには対象件数（`対象 24 記事` / `未翻訳 5 タグ`。`GET /tags/bulk-status` が返す）を出し、0 件なら無効にする——旧ラベルは処理内容も残件も示しておらず、実際には保管 1881 件のうち 1857 件に既にタグが付いていて「ほぼ対象なし」なのが分からなかった
