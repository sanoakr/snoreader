...
- キーワードによる自動タグ付けは明示操作でのみ行い、保管の副作用としては付けない。スターを付けてもタグは増えず、同じマッチ結果はリーダーの青い「Suggested:」チップに出るのでタップ 1 回で付けられる。「タグ」⚙ のモーダルの `Auto tag` は Saved 記事に対して一括でマッチを走らせる（未タグは最大 3 件付与、4 件以上付いている記事は既存タグを剥がして再付与）
