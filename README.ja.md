...
- リーダーのレイアウトを動かさない——抽出時に本文画像すべての `width`/`height` を埋める（元ページの宣言値を優先し、無ければ Range リクエストで画像ヘッダーから実測する。実測先は公開アドレスの 80/443 のみに限定しリダイレクトも追わない——取得したページの `<img src>` は攻撃者が書ける値なので、ループバックや LAN 内サービスへのリクエストに使われないようにするため）。読み込み前から領域が予約されるので、記事を読んでいる最中にスクロール位置がずれない。大きい GIF（およそ 200×200 以上）は自動読み込みせず、実画像とまったく同じ寸法の「▶ GIF を再生」プレースホルダをタップしてから再生する（GIF は放置中もデコードが走り続け、iOS 系ブラウザで画面全体の再描画＝点滅を招くため）。アイコン程度の GIF はそのまま表示し、1×1 のビーコンや遅延読み込みスペーサー（Togetter / Posfie は 1 ページに数十枚並べる）は寸法が判った時点で削除する
- 日英バイリンガルタグ——日英表示切り替え・手動入力時の自動翻訳
- AI 要約自動生成（バックグラウンドジョブ、日本語箇条書き）
- AI タグ提案——既存タグとのキーワードマッチ（タイトル / 本文、Unicode セーフ）と LLM 候補をマージ
- キーワードによる自動タグ付けは明示操作でのみ行い、保管の副作用としては付けない。スターを付けてもタグは増えず、同じマッチ結果はリーダーの青い「Suggested:」チップに出るのでタップ 1 回で付けられる。「タグ」⚙ のモーダルの `Saved に既存タグ付け` は Saved 記事に対して一括でマッチを走らせる（未タグは最大 3 件付与、4 件以上付いている記事は既存タグを剥がして再付与）。隣の `Saved に AI タグ付け` は LLM の経路（タグの無い Saved 記事を 1 回 10 件、新しいタグ名も作る）、`タグの日本語名を補完` は日本語名の穴埋め。ラベルに対象を書くのは、タグ付けの 2 つがどちらも Saved 記事だけを触り、違いは `既存` か `AI` かだけだから。各ボタンには対象件数（`対象 24 記事` / `未翻訳 5 タグ`。`GET /tags/bulk-status` が返す）を出し、0 件なら無効にする——旧ラベルは処理内容も残件も示しておらず、実際には保管 1881 件のうち 1857 件に既にタグが付いていて「ほぼ対象なし」なのが分からなかった
- 記事単位の LLM チャットパネル（記事本文・DuckDuckGo Web 検索・一般知識を併用して回答）
- チャットの質問候補 — 記事に即した短い質問をチップで入力欄の上に表示し、タップでそのまま送信できる。バックグラウンドで先回り生成（未読を優先）して記事ごとにキャッシュするため、記事を開いた時点でチップが並んでいる。チップから送ると会話を踏まえた次の候補に入れ替わり、手入力で質問したときは候補をそのまま出し続ける
- IDF 重み付き「Recommend」ビュー（保存記事の 20% 超に付いているタグは、好みを判別できない汎用タグとみなして自動除外）。保存記事と共通するタグが**2 本以上**残ることを条件にし、偶然 1 本かすっただけの記事は推薦しない。スコアの下限値ではなく本数で表しているのは、スコアの値域が保存記事の件数とともに広がるため、絶対値の下限は保存が貯まるほど効かなくなるから（サイドバー順: All / Recommend / Saved）
...
- サブジャンル——ジャンルは 1 段だけ子を持てる。親の未読が 30 件を超えるとサイドバーが子を展開し、どの束も「片付けられる大きさ」に見えるようにする（閾値以下なら従来通り 1 行。ただしその親配下を読んでいる間は展開したままで、選んだ行が消えない）。タグのルールは親でも子でも指せて、解決はより具体的な方を優先するので、ジャンルの代表タグを子に降ろせば実際に束が割れる。親を選ぶと子の記事も含まれ、「その他」行を選ぶと親に直接分類された記事だけになる。30 を超えた束は件数バッジが amber になる（タグ分布が偏っているジャンルは意味では割り切れないため）。「ジャンル管理」の「推奨サブジャンルを投入」で、AI・LLM と開発・技術に実データから作った既定の分割を入れられる（冪等。押し直すと足りない分だけ入る）
- ジャンルの自動分割提案——ジャンルの未読が 50 件（`SNOREADER_GENRE_UNREAD_LIMIT`）を超えると、毎時の取得サイクルが分割案を作り、「ジャンル管理」に件数バッジ付きで並べる。試す手は 3 つ。受け皿になっているタグを汎用ルールに落として、より具体的なジャンルに取り返させる／ジャンルの担当タグを新しい兄弟に分ける／よく共起するのにまだルールが無いタグを新しいジャンルの担当にする。**表示される件数はすべて、提案後の辞書で実際に未読を分類し直した実測値**で、推定値は出さない（LLM は分類に使わない。使うのは日本語ラベルの命名だけ）。1 つの案について「上限を超えたジャンル自身が何件まで下がるか」と「影響したジャンルの中で最悪が何件になるか」の両方を出すのは、後者だけを見せると 35 件の改善が 3 件の改善に読めてしまうため。**自動では適用しない**——1 件ずつ内容を見て、提案された日本語ラベルを直してから適用する（辞書を書き換えて既存記事を再分類するので 1 分ほどかかる）。「無視」するとそのジャンルが更に増えるまで再提案されず、「適用」すると同ジャンルの他の案は消える（辞書が変わった時点で他の案の件数は無効になるため）
- 非表示（dismissed）——非表示は既読化ではないので、保持期間による自動削除の対象にならない。「非表示にした記事」ビューや検索から戻せる（検索結果では 非表示 バッジが付く）
- ジャンル辞書の編集——サイドバーの「ジャンル」見出しの ⚙ から開く「ジャンル管理」（画面中央のオーバーレイ。辞書エディタは幅 256px のサイドバーに収まらないため。✕ / Escape / 背景クリックで閉じ、開いている間はリストのショートカットを止める）で、ジャンルの追加・削除・表示名変更・優先順位の変更（タグが複数ジャンルに当たったときどちらが勝つか）・タグの移動ができる。変更するたびに既存記事が再分類され、何件動いたかが表示される。分類できなかった記事は「その他」ジャンルから辿れるので、実データを見ながら辞書を育てられる
...
- フィード横断の重複記事整理——正規化 URL（トラッキングパラメータ・フラグメント除去）が一致する記事を重複とみなし、保管済み > 非はてなブックマーク由来 > 取得日時が古い方の優先順位で 1 件を残し、削除される側の既読・保管状態やタグはマージする。定期取得のたびに自動実行、サイドバーの「フィード」見出しの ⚙ にある「重複記事を整理」からドライラン確認付きで手動実行も可能
- 管理操作は対象の隣に置く——「タグ」「ジャンル」「フィード」の各見出しの ⚙ が、その見出しの管理だけを開く。「タグ」（名前の変更・削除・一括タグ付け）と「ジャンル」（辞書と分割提案）はどちらも幅 256px に収まらないので画面中央のモーダルで開き、「フィード」（重複記事の整理・除外パターン・OPML の入出力）はその場に 1 行で開く。Saved 記事の JSON の入出力は ★ Saved の直下に置き、`+ Add Feed` はフィード一覧の最後の行に置く
...
候補は `articles.chat_suggestions` にキャッシュされ、`GET /api/articles/{id}/chat-suggestions` が返す。このエンドポイントは既定では LLM を呼ばず、記事を開いたときはキャッシュを読むだけである。キャッシュはバックグラウンド処理の **Phase 3** が先回りで埋める（要約済みの記事を順に処理する）ので、通常はパネルを開いた時点でチップが並んでいる。Phase 3 はキュー最低優先度 `PRIORITY_IDLE` で `bulk` レーンに載るため、要約・タグ生成（`PRIORITY_BACKGROUND`）が残っている限り必ずそちらが先に処理され、`reserved` レーンのフォアグラウンド要求には一切影響しない。処理順は 未読 → 保存済み → 新しい順 で、チャットを開くのはこれから読む記事が多いためこの順にしている。残件数はサイドバーの AI ステータス行に `質問候補 N件` として出る。

未生成の記事（取り込み直後、またはモデルが使える候補を返せなかった記事）ではチップの代わりに **✨ 質問候補を生成** ボタンを表示し、そのクリックで `?generate=true` を投げる。ウォーム状態のローカルモデルで生成は約 8 秒かかるが、保存されるので次回以降は即座に表示される。
...
3. SnoReader サイドバーの **Import Saved**（★ Saved の直下）からアップロード
