...
- IDF 重み付き「Recommend」ビュー（カバー率 30% 以上のタグを自動除外）。保存記事と共通するタグが**2 本以上**残ることを条件にし、偶然 1 本かすっただけの記事は推薦しない。スコアの下限値ではなく本数で表しているのは、スコアの値域が保存記事の件数とともに広がるため、絶対値の下限は保存が貯まるほど効かなくなるから（サイドバー順: All / Recommend / Saved）
