...
- リーダーのレイアウトを動かさない——抽出時に本文画像すべての `width`/`height` を埋める（元ページの宣言値を優先し、無ければ Range リクエストで画像ヘッダーから実測する。実測先は公開アドレスの 80/443 のみに限定しリダイレクトも追わない——取得したページの `<img src>` は攻撃者が書ける値なので、ループバックや LAN 内サービスへのリクエストに使われないようにするため）。読み込み前から領域が予約されるので、記事を読んでいる最中にスクロール位置がずれない。アニメ GIF は自動読み込みせず、同じ高さの「▶ GIF を再生」プレースホルダをタップしてから再生する（GIF は放置中もデコードが走り続け、iOS 系ブラウザで画面全体の再描画＝点滅を招くため）
