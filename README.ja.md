...
- 管理操作は対象の隣に置く——「タグ」「ジャンル」「フィード」の各見出しの ⚙ が、その見出しの管理だけを開く（タグ名の変更と一括タグ付け／ジャンル辞書と分割提案／重複記事の整理・除外パターン・OPML の入出力）。Saved 記事の JSON の入出力は ★ Saved の直下に置き、`+ Add Feed` はフィード一覧の最後の行に置く
