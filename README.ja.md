...
- キーワードによる自動タグ付けは明示操作でのみ行い、保管の副作用としては付けない。スターを付けてもタグは増えず、同じマッチ結果はリーダーの青い「Suggested:」チップに出るのでタップ 1 回で付けられる。サイドバー ⚙ メニューの `Auto tag` は Saved 記事に対して一括でマッチを走らせる（未タグは最大 3 件付与、4 件以上付いている記事は既存タグを剥がして再付与）
...
- 管理操作は対象の隣に置く——「タグ」「ジャンル」「フィード」の各見出しの ⚙ が、その見出しの管理だけを開く（タグ名の変更と一括タグ付け／ジャンル辞書と分割提案／重複記事の整理と除外パターン）。入出力も対象の隣で、Saved 記事の JSON は ★ Saved の直下、OPML はフィード一覧の末尾（+ Add Feed の隣）に置く
