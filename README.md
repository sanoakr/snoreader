...
- Suggested chat questions — tappable chips of short, article-specific questions above the chat input. Generated on demand with one explicit click and cached per article, so simply opening an article never costs an LLM call. Sending a chip refreshes the chips into conversation-aware follow-ups; typing your own question leaves them as they are
