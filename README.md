...
- IDF-weighted "Recommend" view with automatic exclusion of high-coverage tags (coverage ≥ 30%). An article needs at least **two** surviving tags in common with your saved articles — a single chance overlap does not qualify it. The requirement is a tag count rather than a score floor because the score range grows with the size of the saved corpus, so any absolute floor stops discriminating as saved articles accumulate (sidebar order: All / Recommend / Saved)
