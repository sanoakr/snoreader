...
- Keyword auto-tagging is an explicit action, never a side effect of saving. Starring an article attaches nothing on its own — the same keyword matches show up as blue "Suggested:" chips in the reader, one tap each. The タグ ⚙ modal's `Saved に既存タグ付け` runs the match over Saved articles in bulk: untagged ones get up to 3 tags, and articles with 4 or more have their tags stripped and rebuilt. Its neighbour `Saved に AI タグ付け` is the LLM path (10 tag-less Saved articles per click, new tag names allowed) and `タグの日本語名を補完` fills in Japanese names. Each label names its object, since both tagging actions touch Saved articles only and the difference between them is `既存` vs `AI`. Each button carries the number of items it would touch (`対象 24 記事` / `未翻訳 5 タグ`, from `GET /tags/bulk-status`) and is disabled at zero — the old `Auto tag` / `AI tag` labels said neither what they did nor whether there was anything left to do, and with tags already attached to 1,857 of 1,881 saved articles the answer is usually "almost nothing"
...
- Automatic split suggestions — when a genre's unread count goes over 50 (`SNOREADER_GENRE_UNREAD_LIMIT`), the hourly fetch computes proposals for subdividing it and `ジャンル管理` shows them with a count badge. Three approaches are tried: demote a catch-all tag so more specific genres win it back, split a genre's own tags into new siblings, or adopt a frequently co-occurring tag that has no rule yet. Every number shown is measured by re-classifying the actual unread set against the proposed dictionary, never estimated — a proposal reports both what the over-limit genre itself drops to and what the worst affected genre becomes. Proposals are **never applied automatically**: you review each one, edit the suggested Japanese label, and apply it with one click (which rewrites the dictionary and reclassifies existing articles, so it takes around a minute). Dismissing one stops it coming back until that genre grows further; applying one clears the rest, since a dictionary change invalidates their numbers
...
- Editable genre dictionary — add/remove genres, rename them, reorder priority (which genre wins when an article's tags hit several), and move tags between genres in the ジャンル管理 modal, opened from the sidebar's ジャンル ⚙ (a centered overlay — the dictionary editor does not fit the 256px sidebar; ✕, Escape or a backdrop click closes it, and the list shortcuts are suspended while it is open). Every change reclassifies existing articles immediately and reports how many moved; unclassified articles are reachable as the "その他" genre so the dictionary can be grown from real data
...
- Sidebar management sits next to what it manages — each section heading (タグ / ジャンル / フィード) carries a ⚙ for its own tools only. タグ and ジャンル open a centered modal (tag rename and bulk tagging / the genre dictionary and split suggestions), since neither fits a 256px column; フィード unfolds a single row in place (duplicate cleanup, exclude patterns, OPML import/export). Saved-article JSON import/export sits directly under ★ Saved, and `+ Add Feed` is the last row of the feed list
...
3. In the SnoReader sidebar, click **Import Saved** (right under ★ Saved) and upload the file
