...
- Keyword auto-tagging is an explicit action, never a side effect of saving. Starring an article attaches nothing on its own — the same keyword matches show up as blue "Suggested:" chips in the reader, one tap each. The タグ ⚙ modal's `キーワードで付与` runs the match over Saved articles in bulk: untagged ones get up to 3 tags, and articles with 4 or more have their tags stripped and rebuilt. Its neighbour `AI でタグを生成` is the LLM path (10 tag-less Saved articles per click, new tag names allowed) and `日本語名を補完` fills in Japanese names. Each button carries the number of items it would touch (`対象 24 記事` / `未翻訳 5 タグ`, from `GET /tags/bulk-status`) and is disabled at zero — the old `Auto tag` / `AI tag` labels said neither what they did nor whether there was anything left to do, and with tags already attached to 1,857 of 1,881 saved articles the answer is usually "almost nothing"
