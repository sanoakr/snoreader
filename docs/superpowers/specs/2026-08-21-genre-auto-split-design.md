...
class SplitProposal:
    ...
    projected_target: int    # 対象ジャンル genre_key 自身の適用後件数。projected_max とは別物
                              # （demote_generic では対象は大きく縮む一方、譲られた既存ジャンルが
                              # わずかに上限を超えることがあり、その場合 projected_max は譲られた
                              # 側の件数になる）。UI は両方を出す——projected_max だけ見せると
                              # 35 件の改善が 3 件の改善に見えることがある
...
corpus 全体の最大値を採ると、**無関係なジャンルの増減だけで正しい案の合否が
狂う**（戦略 C は「影響先バケットの最大値が適用前より厳密に改善したか」を
`_affected_max(current, projected, genre_key) < before_max` で見るため）。本番では
`other` が 46 件あり、これが 51 に育った瞬間に「corpus 全体の最大値」は 46→51 で
悪化したように見え、無関係なはずの `ai_misc` の降格案（実際には改善している）まで
判定を狂わされる、という形で仕組み全体が無言で止まる。実装は
`_affected_max(current, projected, genre_key)` に閉じ込める。

なお採用条件そのものも「影響先バケットが全部 limit 以下」ではなく「影響先の
最大値が適用前と比べて厳密に改善している」である。前者は本番データで実際に
満たせなかった: `ai_misc`（54 件）は `ai` を降格すれば 19 件まで下がる劇的な
改善だが、再配分先の `politics` が 46 → 51 でちょうど limit を超えるため、
「全員 limit 以下」を要求すると 54 → 19 という明白な改善案まで丸ごと棄却され、
何も提案されなくなる。「厳密に改善していること」であれば「一つ潰して一つ増やす」
だけの悪化は防げるし、新たに超過した `politics` 自身は次サイクルで自分の提案対象に
なる——これはこの機能が意図する反復的な挙動そのもの。人間が 1 件ずつ承認するので、
往復のような振動が起きても人が止める余地がある。
...
| `_DEMOTE_COVERAGE` | 0.8 | 戦略 C（`demote_generic`）がタグを「受け皿」と見なす、対象ジャンルの未読に対するカバー率の下限 |
...
**`dismiss` と `apply` は後始末の仕方が違う。** `dismiss` はその `genre_key` の
保留中の行すべてに `dismissed_at` を立てる（1 つの案だけ消えて残りが居座るのは
「無視した」という意思に反する）。`dismissed_at_count` はそこにフロアを立てる——
「ユーザーが断った時点の件数より増えるまで再提案しない」という記憶である。

`apply` は違う。辞書が変わった瞬間、対象ジャンルだけでなく**全ジャンル**の
保留中の提案の `projected_max`／`projected_target` が無効になる（`demote_generic`
は対象ジャンルのタグを他ジャンルへ再配分するので、対象以外の保留提案の実測値も
同時に古くなる）。そこで `apply` は対象ジャンル以外も含めて保留中の行を**全部
削除する**——`dismiss` のようにフロアを立てて残すことはしない。理由は、
「断られた」のではなく「適用に成功した」ケースだからだ。もしここで
`dismissed_at`／`dismissed_at_count` を立てると、その時点の件数が
成功した適用のフロアになってしまい、`limit` を超えてから適用直前の件数まで
の帯 `(limit, 適用直前の件数]` の間、そのジャンルが再提案されず沈黙する
（実際に end-to-end で確認済み）。単に消せば、次に上限を超えたときに
何の制約もなく新しい提案が作られる。
...
- 単一タグの受け皿が上限超 → 戦略 C が降格を提案し、シミュレート後の影響先最大値が適用前より厳密に改善する（影響先が全員 limit 以下になるとは限らない）
...
- 同一ジャンルに複数案があるとき、1 つ適用すると残り（他ジャンルの保留分も含む）は削除される——`dismiss` と違い閉じる（フロアを立てる）のではない
